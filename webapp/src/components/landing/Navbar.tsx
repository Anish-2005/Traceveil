                    ...
                    <div className={`flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-12' : 'h-16'}`}>
                        ...
                        <Link href="/" className="flex items-center gap-3 group relative z-50" onClick={() => setMobileMenuOpen(false)}>
                            ...
                            <div className="flex flex-col">
                                <span className={`font-bold app-brand-wordmark tracking-tight transition-all duration-300 ${scrolled ? 'text-base sm:text-lg' : 'text-xl sm:text-2xl'}`}>
                                    Traceveil
                                </span>
                                <span className={`hidden lg:block text-[10px] app-text-muted uppercase tracking-[0.14em] transition-all duration-300 ${scrolled ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
                        ...
                        {/* Desktop Nav Links */}
                        <div className={`hidden lg:flex items-center transition-all duration-300 ${scrolled ? 'gap-0.5 px-1 py-1 rounded-full app-nav-pill' : 'gap-1'}`}>
                            {navLinks.map((link) => (
                                <Link
                                    key={link.href}
                                    href={link.href.startsWith('#') && pathname !== '/' ? `/${link.href}` : link.href}
                                    className={`font-medium app-landing-link rounded-full transition-all ${scrolled ? 'px-3 py-1.5 text-xs' : 'px-4 py-2 text-sm'}`}
                                >
                                    {link.label}
                                </Link>
                            ))}
                        ...
                        <div className="hidden md:flex items-center gap-3">
                            ...
                            {!user ? (
                                <>
                                    <Link
                                        href="/login"
                                        className={`font-medium app-landing-link transition-colors ${scrolled ? 'px-3 py-1.5 text-xs' : 'px-4 py-2 text-sm'}`}
                                    >
                                        Sign In
                                    </Link>
                                    <Link
                                        href="/dashboard"
                                        className={`group app-cta-pill flex items-center gap-2 rounded-full font-semibold transition-all ${scrolled ? 'pl-3 pr-1.5 py-1 text-xs' : 'pl-5 pr-2 py-1.5 text-sm hover:scale-105'}`}
                                    >
                                        <span>{scrolled ? 'Start' : 'Get Started'}</span>
                                        <div className={`rounded-full app-cta-orb flex items-center justify-center group-hover:rotate-45 transition-transform duration-300 ${scrolled ? 'w-5 h-5' : 'w-7 h-7'}`}>
                        ...
                        <button
                            ...
                            className="lg:hidden relative z-50 p-2 app-icon-btn transition-colors"
                ...
                <div className="h-full flex flex-col pt-24 px-6 pb-8 relative z-10 overflow-y-auto">
                    ...
                    <nav className="flex-1 flex flex-col gap-2">
                        {[
                            ...
                            >
                                <div>
                                    <span className="block text-2xl font-light tracking-tight app-mobile-link-title transition-colors">
                                        {link.label}
                                    </span>
                                    <span className="text-xs app-text-muted font-medium tracking-wide uppercase mt-1 block group-hover:text-slate-400 transition-colors">
                                        {link.desc}
                                    </span>
                                </div>
                    ...
                    >
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-4">
                            <div className="p-4 rounded-2xl app-mobile-stat border">
